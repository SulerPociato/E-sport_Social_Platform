        ...
        <form id="postOrderForm">
          ...
          <div class="order-post-form-group">
            ...
            <select id="orderType" name="orderType" class="order-post-form-input" required>
              ...
              <option value="0">求职订单(job)</option>
              <option value="1">悬赏订单(bounty)</option>
            </select>
          </div>
  ...
  <div class="show-order-grid">
    ...
    <c:choose>
      <c:when test="${not empty orderList}">
        <c:forEach var="order" items="${orderList}" varStatus="status">
          <c:set var="cardClass" value="show-order-card-${status.index % 2 + 1}"/>
          <c:set var="orderType" value="${status.index % 2 == 0 ? 'job' : 'bounty'}"/>
          <c:set var="orderTitle" value="${status.index % 2 == 0 ? '求职订单' : '悬赏订单'}"/>

          <div class="${cardClass}" data-type="${orderType}" data-order-id="${order.orderId}">
            <div class="show-order-card-content">
              <h3>${orderTitle}</h3>
              <div class="show-order-info">
                <div class="show-order-type">${order.gameName}</div>
                <div class="show-order-amount">订单金额：¥${order.totalAmount}</div>
                ...
                <div class="show-order-publisher-info">
                  <div class="show-order-publisher-avatar">
                    <c:choose>
                      <c:when test="${not empty order.customer and not empty order.customer.avatar}">
                        <img src="${order.customer.avatar}" alt="发布人头像">
                      </c:when>
                      <c:otherwise>
                  ...
                  <div class="show-order-publisher-details">
                    <c:choose>
                      <c:when test="${not empty order.customer}">
                        <div class="show-order-publisher-name">${order.customer.nickname}</div>
                        <div class="show-order-publisher-uid">UID: ${order.customer.userId}</div>
                      </c:when>
                      <c:otherwise>
                        <div class="show-order-publisher-name">匿名用户</div>
                        <div class="show-order-publisher-uid">UID: ${order.customerId}</div>
                      </c:otherwise>
                    </c:choose>
                  </div>
                </div>
              </div>
              <button class="show-order-accept-btn" onclick="acceptOrder(${order.orderId})">接受订单</button>
            </div>
          </div>
        </c:forEach>
      </c:when>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (postOrderForm) {
      postOrderForm.addEventListener('submit', function(event) {
        ...
        const detail = document.getElementById('orderDetails').value.trim();
        ...
        orderData.append('orderType', orderType); // 添加订单类型参数
...
// 弹窗内容（不再需要绝对定位和 transform）
      qrModal.innerHTML = `
    <div style="background:#222; border-radius:16px; width:300px; padding:20px; text-align:center; position:relative;">
